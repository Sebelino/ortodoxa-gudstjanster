...
// generateIcon renders the Orthodox cross icon at the given size.
// Mirrors the SVG favicon: background with rounded corners, vertical shaft,
// top bar (titulus), main crossbeam, and diagonal footrest.
func generateIcon(size int) *image.RGBA {
	img := image.NewRGBA(image.Rect(0, 0, size, size))
	s := float64(size) / 32.0

	fillRoundedRect(img, 0, 0, float64(size), float64(size), 4*s, bgColor)
	fillRect(img, 14*s, 3*s, 4*s, 26*s, crossColor)       // vertical shaft
	fillRect(img, 9*s, 6*s, 14*s, 3.5*s, crossColor)      // top bar
	fillRect(img, 4*s, 13*s, 24*s, 4*s, crossColor)        // main crossbeam
	drawLine(img, 9*s, 23*s, 23*s, 26.5*s, 3*s, crossColor) // diagonal footrest
...
func serveIcon(w http.ResponseWriter, size int) {
	if cached, ok := iconCache.Load(size); ok {
